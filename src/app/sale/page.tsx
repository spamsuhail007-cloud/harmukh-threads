...
// ─── Product Card (WhatsApp-CTA variant) ──────────────────────────────────
function LandingProductCard({ product }: { product: Product }) {
  const waMsg = `Hi! I'm interested in *${product.name}* (${formatPrice(product.price)}). Can you share more details? 🙏`;
  ...
  return (
    <div style={{
      ...
      {/* Image */}
      <div style={{ position: 'relative', aspectRatio: '1/1', overflow: 'hidden', background: '#f0eee7' }}>
        <Image
        ...
        {isOOS && (
          <div style={{
            ...
            <span style={{ color: '#fff', fontWeight: 700, fontSize: '0.85rem', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Sold Out</span>
          </div>
        )}
        {product.badge && !isOOS && (
          <span style={{
      ...
      {/* Body */}
      <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '6px', flex: 1 }}>
        <div style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#765a24' }}>
          {product.category}
        </div>
        <h3 style={{ fontFamily: 'var(--font-serif, serif)', fontSize: '0.95rem', fontWeight: 700, color: '#1c1c18', lineHeight: 1.3, margin: 0 }}>
          {product.name}
        </h3>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '2px' }}>
          <span style={{ fontWeight: 700, color: '#9b4000', fontSize: '1rem' }}>{formatPrice(product.price)}</span>
          {product.originalPrice && (
            <del style={{ fontSize: '0.8rem', color: '#8b7266' }}>{formatPrice(product.originalPrice)}</del>
          )}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '2px' }}>
          {'★★★★★'.split('').map((s, i) => (
            <span key={i} style={{ color: '#f59e0b', fontSize: '0.75rem' }}>{s}</span>
          ))}
          <span style={{ fontSize: '0.7rem', color: '#8b7266', marginLeft: '2px' }}>Artisan Made</span>
        </div>
        ...
        <a
          ...
          style={{
            ...
            gap: '6px',
            background: isOOS ? '#e5e2db' : '#25D366',
        ...
        >
          ...
          {isOOS ? 'Sold Out' : 'Enquire on WhatsApp'}
        </a>
        ...
        {!isOOS && (
          <Link
            ...
            style={{ textAlign: 'center', fontSize: '0.7rem', color: '#8b7266', textDecoration: 'underline', marginTop: '4px' }}
          >
            View full details →
          </Link>
        )}
